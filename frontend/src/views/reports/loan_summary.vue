<template>
  <div>
    <el-card>
      ...
      <h5>Loan with balance Report</h5>
      <hr />
...
export default {
  ...
  async created() {
    try {
      ...
      this.datas.forEach(el => {
        let sched = new Date(el.loanscheds[0].date);
        let day = this.days.find(x => x.code == sched.getDay()).name;
        if (el.payment_period != 3) {
          el.schedule =
            this.periods.find(e => e.code === el.payment_period).name +
            "-" +
            day;
          console.log(
            this.periods.find(e => e.code === el.payment_period).name
          ...
          el.schedule = this.periods.find(
            e => e.code === el.payment_period
          ).name;
        }
      });
    } catch (err) {
